...
**This is primarily a _qualitative_ study.** Although we collect quantitative data, its primary use is to categorise the qualitative feedback received. Secondary to this is it's use as a broad, high level overview of the survey to provide a context for the textual analysis to exist in. As such we do not report any summary statistics, such as means, but simply present the number of reponses for each question in a frequency table.
...
Here we have overlayed the responses received for each question in order to get a
...
to create this plot we have had to make the assumption that the distance between each response
category is uniform.

Plotting grouped bar charts of the responses to each question would be a more accurate
representation of the data, but this would require positioning the questions next to each
other rather than overlaying them. This would be difficult to interprete visually.
