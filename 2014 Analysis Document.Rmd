...
December, and reponses are collected up until min Janurary. Within this period
...
~80 employees. While we don't have employee numbers for corresponding periods in
previous years, this almost certainly represents the highest response rate
received yet.

As our sample size approaches our population size, we become more confident that
...
represents close to 75% of the population. 

Something that *would* increase the representativeness of our sample is a test
for non-response bias. We are open to ideas on how best this could be done, whilst
maintaining the annonymity of the survey.


# Methodology: quantative data

Any analysis of quantative data first needs to consider the nature of the data itself in order to draw statistically valid conclusions from it. Stanley Smith Stevens'[^sss1946] *Theory of Scales of Measurment* proposes a heirarchy of four levels:
...
\newthought{With the caveats mentioned above in mind,} we present a brief high level overview of the data to provide a context for the qualitative analysis in the next section. In these initial sections we will also print the R commands used to generate the data for the purposes of transparency. As mentioned earlier, the R commands for this whole report can be seen in the  associated R Markdown file used to create this PDF.
...
As can be seen in the right hand column, the response rates for each question remain $\ge$ 55 as indicated in grey, with the number of people not providing an answer indicated in red. (Presumably the higher non-response rate for "I enjoy AC staff camps" is due to the number of new staff hired over the year who have not yet attend a staff camp.)

For this next plot we have overlayed the responses received for each question in order to get a
...
as an initial step in exploring the data. Its primary limitations stem from the
assumptions this plot makes.

In order to plot discrete data points on a continuous scale, we need to interpolate between data points.
